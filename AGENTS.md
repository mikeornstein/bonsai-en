...
- Share: image-first panel (⋯ → Share) — **Messages / system share**, **Post to X** (short caption + portrait PNG, no `#s=` hash in the draft), **Save image**, and optional **Copy restore link** (full LZ hash URL; long by design, not shown in the UI). Never put tree deep-links in social compose text (#90). Clipboard failure is never reported as “tree too large.”  
